...
const Toast: React.FC<{ message: string; visible: boolean; type?: 'info' | 'success' }> = ({ message, visible, type = 'info' }) => (
  <div className={`fixed bottom-8 left-1/2 -translate-x-1/2 z-[100] px-6 py-3 rounded-2xl shadow-2xl transition-all duration-300 pointer-events-none flex items-center gap-3 ${visible ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0'} ${type === 'success' ? 'bg-emerald-600 text-white' : 'bg-slate-900 text-white'}`}>
    {type === 'success' ? (
...
const EventCard: React.FC<{ 
  ...
  const shareText = `Check out "${event.title}" on EventPulse SA!`;
  ...
  return (
    <div className={`group relative bg-white rounded-3xl border ${isAI ? 'border-indigo-200 shadow-indigo-50 shadow-xl' : 'border-slate-200 shadow-sm'} overflow-hidden hover:shadow-xl transition-all duration-500 ${event.status === 'flagged' ? 'opacity-75 grayscale-[0.5]' : ''}`}>
      <div className="relative h-52 overflow-hidden">
        ...
        <div className="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-black/80 to-transparent translate-y-full group-hover:translate-y-0 transition-transform duration-300 flex justify-center gap-3">
          <a href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(shareText)}&url=${encodeURIComponent(shareUrl)}`} target="_blank" rel="noreferrer" onClick={() => onShare(event.id)} className="bg-sky-500 p-2 rounded-xl text-white hover:scale-110 transition-transform shadow-lg"><svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.84 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z"/></svg></a>
          <a href={`https://api.whatsapp.com/send?text=${encodeURIComponent(shareText + ' ' + shareUrl)}`} target="_blank" rel="noreferrer" onClick={() => onShare(event.id)} className="bg-emerald-500 p-2 rounded-xl text-white hover:scale-110 transition-transform shadow-lg"><svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg></a>
          <a href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl)}`} target="_blank" rel="noreferrer" onClick={() => onShare(event.id)} className="bg-blue-600 p-2 rounded-xl text-white hover:scale-110 transition-transform shadow-lg"><svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg></a>
        </div>
      </div>
...
const Dashboard: React.FC<{
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-12">
      ...
      >
        <p className="text-slate-600 mb-8 font-medium leading-relaxed">This action will update the public record for this event. Are you absolutely sure?</p>
      ...
      <div className="flex flex-col md:flex-row items-center justify-between mb-16 gap-6">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-2">Manage your events and preferences</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        <section className="lg:col-span-1 space-y-10">
          <div className="bg-white p-10 rounded-[3rem] border border-slate-200 shadow-sm overflow-hidden relative">
            ...
            <p className="mt-8 text-[10px] text-slate-400 font-bold uppercase tracking-widest leading-relaxed">Personalized AI recommendations update based on these categories.</p>
          </div>
          ...
          <div className="bg-slate-900 p-10 rounded-[3rem] shadow-2xl text-white">
            ...
            ) : <p className="text-slate-500 font-bold text-sm text-center py-6 border border-white/5 rounded-3xl">Ready to host your first event?</p>}
          </div>
        </section>
...
const App: React.FC = () => {
  ...
  const [toast, setToast] = useState<{ message: string; visible: boolean; type?: 'info' | 'success' }>({ message: '', visible: false });
  ...
  useEffect(() => {
    ...
    const savedCustomEvents = localStorage.getItem('pulse_custom_events_v3');
    if (savedUser) setUser(JSON.parse(savedUser));
    if (savedCustomEvents) {
      const custom = JSON.parse(savedCustomEvents);
      // Ensure no duplicates with MOCK_EVENTS
      const filteredMocks = MOCK_EVENTS.filter(me => !custom.some((ce: Event) => ce.id === me.id));
      setEvents([...custom, ...filteredMocks]);
    }
  }, []);

  useEffect(() => {
    if (user) localStorage.setItem('pulse_user_v3', JSON.stringify(user));
    const custom = events.filter(e => !MOCK_EVENTS.some(m => m.id === e.id));
    localStorage.setItem('pulse_custom_events_v3', JSON.stringify(custom));
  }, [user, events]);

  // View logic
  useEffect(() => {
    if (view === 'Detail' && !selectedEventId) setView('Home');
  ...
  const handleLogin = (role: UserRole = 'user') => {
    const mockUser: User = {
      ...
      email: role === 'admin' ? 'admin@eventpulse.co.za' : 'lerato@pulse.co.za',
  ...
  const handleInteraction = (eventId: string, type: 'view' | 'save' | 'share') => {
    ...
    if (type === 'save') {
      ...
      showToast(isCurrentlySaved ? "Pulse removed from hearts." : "Pulse hearted!", 'success');
    }
  ...
  const handleReminderToggle = (eventId: string) => {
    if (!user) return;
    const hasRem = user.reminders.includes(eventId);
    const updated = hasRem 
      ? user.reminders.filter(id => id !== eventId)
      : [...user.reminders, eventId];
    
    setUser({ ...user, reminders: updated });
    showToast(hasRem ? "Pulse alert cancelled." : "Pulse alert active!", 'success');
  };

  const handleUpdateInterests = (newInterests: Category[]) => {
    if (!user) return;
    setUser({ ...user, interests: newInterests });
    showToast("Interests updated! Refreshing AI picks...");
  };

  const handleAddEvent = (data: Omit<Event, 'id' | 'organizer' | 'organizerId'>) => {
    if (!user) return;
    ...
    showToast("Pulse hosted! It's now live.", 'success');
  };

  const handleEditEvent = (ev: Event) => {
    setEvents(events.map(e => e.id === ev.id ? ev : e));
    showToast("Pulse updated successfully.", 'success');
  };
  ...
  const handleModeration = (id: string, action: 'approve' | 'flag' | 'delete') => {
    if (action === 'delete') {
      handleDeleteEvent(id);
    } else {
      setEvents(events.map(e => e.id === id ? { ...e, status: action === 'approve' ? 'approved' : 'flagged' } : e));
      showToast(action === 'approve' ? "Pulse approved." : "Pulse flagged for review.");
    }
  };
  ...
  return (
    <div className="min-h-screen pb-20 selection:bg-indigo-100 selection:text-indigo-900 overflow-x-hidden">
      <Navbar 
        user={user} 
      ...
      <main className="animate-in fade-in duration-700">
        {(view === 'Home' || !user) ? (
          ...
          <EventDetail 
            ...
            onReminder={handleReminderToggle}
            isSaved={user.savedEvents.includes(selectedEvent.id)}
            hasReminder={user.reminders.includes(selectedEvent.id)}
          />
        ) : null}
      </main>
      ...
      <footer className="mt-20 py-20 border-t border-slate-200 bg-white">
        <div className="max-w-7xl mx-auto px-4 text-center">
          ...
          <p className="text-slate-400 text-[10px] font-black uppercase tracking-[0.4em] mb-2">Designed for the Pulse of Mzansi</p>
          <p className="text-slate-900 font-bold text-sm">© 2026 TBangCode Solution South Africa</p>
        </div>
      </footer>
    </div>
  );
